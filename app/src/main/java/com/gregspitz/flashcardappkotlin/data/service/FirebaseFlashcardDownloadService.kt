            ...
            override fun onDataChange(p0: DataSnapshot?) {
                if (p0 != null) {
                    ...
                    for (category in p0.children) {
                        ...
                        cat?.let {
                            downloadCategories.add(DownloadCategory(it.name, it.count, category.key))
                        }
                    }
            ...
            override fun onDataChange(p0: DataSnapshot?) {
                if (p0 != null) {
                    ...
                    for (flashcard in p0.children) {
                        ...
                        card?.let {
                            flashcards.add(DownloadFlashcard(flashcard.key, card.category_name,
                                    card.category_id, card.front, card.back))
                        }
                    }
